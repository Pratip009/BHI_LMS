...
const CourseContentList: FC<Props> = (props) => {
    ...
    const videoSections: string[] = [
        ...Array.from(new Set<string>(props.data?.map((item: any) => item.videoSections)))
    ]
    ...
    return (
        <div className={`mt-[15px] ${!props.isDemo && 'ml-[-30px] sticky top-24 left-0 z-30 '}`}>
            {videoSections.map((section: string, sectionIndex: number) => {
                const isSectionVisible = visibleSections.has(section);

                const sectionVideos: any[] = props.data.filter((item: any) => item.videoSections === section);
                        ...
                        {isSectionVisible && (
                            <div className="w-full">
                                {sectionVideos.map((item: any, index: number) => {
                                    ...
                                    return (
                                        <div className={`w-full ${videoIndex === props.activeVideo ? "bg-slate-800" : ""} cursor-pointer transition-all p-2`} key={item._id}
