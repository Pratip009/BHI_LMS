...
const CourseContentMedia = ({ data, params, activeVideo, setActiveVideo, userDataMedia, refetch }: Props) => {
    const id = params.id;
    ...
    const [addAnswerInQuestion, { isSuccess: answerSuccess, error: answerError, isLoading: answerCreationLoading }] =
        useAddAnswerInQuestionMutation();
    const [addReviewInCourse, { isSuccess: reviewSuccess, error: reviewError, isLoading: reviewCeationLoading }] =
        useAddReviewInCourseMutation();
    const [addReplyInReview, { isSuccess: replySuccess, error: replyError, isLoading: replyCreationLoading }] =
        useAddReplyInReviewMutation();
    const { data: courseData, refetch: courseRefetch } = useGetCourseDeatilsQuery(id, { refetchOnMountOrArgChange: true });

    const course = courseData?.course;
    const isReviewExist = course?.reviews?.find((item: any) => item.user._id === userDataMedia._id);
            ...
            if ('data' in replyError) {
                ...
                const message =
                    errorMessage.data.message === 'Course not found'
                        ...
                        : errorMessage.data.message === 'Review not found'
            ...
            {activeBar === 2 && (
                <>
                    <div className="flex w-full">
                        <Image
                            src={userDataMedia.avatar ? userDataMedia.avatar.url : userImage}
                            width={50}
                            height={50}
                            alt="user"
                        ...
                        <textarea
                            ...
                            className="dark:text-white text-black outline-none bg-transparent ml-3 border dark:border-[#ffffff57] border-[#5e5e5e7c] 800px:w-full p-2 rounded w-[90%] 800px:text-[18px] font-Poppins"
                        ></textarea>
                    </div>
                                ...
                                <div className="flex w-full">
                                    ...
                                    <div className="w-full">
                                        ...
                                        <textarea
                                            ...
                                            className="outline-none bg-transparent ml-3 border dark:border-[#ffffff57] border-[#4d4d4d57] dark:text-white text-black 800px:w-full p-2 rounded w-[90%] 800px:text-[18px] font-Poppins"
                                        ></textarea>
                                    </div>
                                </div>
                                <div className="w-full flex justify-end">
                                    <div
                                        className={`${styles.button} !w-[120px] !h-[40px] text-[18px] mt-5 800px:mr-0 mr-2 ${reviewCeationLoading && 'cursor-no-drop'
                                            }`}
                                        onClick={reviewCeationLoading ? () => { } : handleReviewSubmit}
                                    >
                                        Submit
                                    </div>
                                </div>
                                ...
                                <div className="w-full my-5" key={index}>
                                    <div className="w-full flex">
                                        ...
                                        <div className="ml-2">
                                            ...
                                            <small className="dark:text-[#ffffff83] text-[#565656FF]">
                                                {format(item.createdAt)}*
                                            </small>
                                        </div>
                                    </div>
                                    {userDataMedia.role === 'admin' && (
                                        <span
                                    ...
                                    {isReviewReply && reviewId === item._id && (
                                        <div className="w-full flex relative">
                                            ...
                                            <button
                                                type="submit"
                                                className={`absolute right-0 bottom-1 dark:text-white text-black ${replyCreationLoading ? 'opacity-50 cursor-not-allowed' : ''
                                                    }`}
                                    ...
                                    {item.commentReplies.map((i: any, index: number) => (
                                        <div className="w-full flex 800px:ml-16 my-5" key={index}>
                                            <div className="w-[50px] h-[50px]">
                                                <Image
                                                    src={i?.user?.avatar?.url || userImage}
                                                    width={50}
                                                    height={50}
                                                    alt="user"
                                                    className="w-[50px] h-[50px] rounded-full object-cover"
                                        ...
                                        {i.user.role === 'admin' && (
                                            <MdVerified className="text-[20px] ml-2 text-[#3d3dec]" />
                                        )}
                                    </div>
                                                <p>{i.comment}</p>
...
const CommentItem = ({ data, setQuestionId, answer, item, setAnswer, handleAnswerSubmit, answerCreationLoading }: any) => {
        ...
        <>
            <div className="my-4">
                <div className="flex mb-2">
                    ...
                    <div className="pl-3">
                        <h5 className="text-[20px] dark:text-white text-black">{item?.user.name}</h5>
                        <p className="dark:text-white text-black">{item?.question}</p>
                        <small className="dark:text-[#ffffff83] text-[#3232328a]">
                ...
                <div className="w-full flex">
                    <span
                        className="800px:pl-16 text-[#3232328a] dark:text-[#ffffff83] cursor-pointer mr-2"
                        onClick={() => {
                    ...
                    <BiMessage
                        size={20}
                        className="cursor-pointer mt-[4px] dark:text-[#ffffff83] text-[#3232328a]"
                    />
                    <span className="pl-1 mt-[-2px] cursor-pointer dark:text-[#ffffff83] text-[#3232328a]">
                        ...
                        <>
                            <div className="w-full flex relative dark:text-white text-black">
                                <input
                                    ...
                                    className={`block 800px:ml-12 mt-2 outline-none bg-transparent dark:text-white text-black border-b border-[#00000027] dark:border-[#fff] p-[5px] w-[95%] ${answer === '' || answerCreationLoading ? 'cursor-not-allowed' : ''
                                        }`}
                                />
                                <button
                                    type="submit"
